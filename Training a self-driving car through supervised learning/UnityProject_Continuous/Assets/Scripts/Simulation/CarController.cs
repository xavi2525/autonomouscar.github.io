...
public class CarController : MonoBehaviour
{
    ...
    private bool DataCreationMode = true;
    ...
    void FixedUpdate()
    {
        ...
        if (!UseUserInput)
        {
            ...
            if (DataCreationMode)
            {
                Movement.CheckInput();//Write the input
                double controlInput = Movement.horizontalInput;
                ...
                if (Input.GetKey("up")) File.AppendAllText(@"C:\Users\Família\Desktop\UnityProject_Continuous\log.txt", sb.ToString());
            }
            else
            {
                //Calling Rstudio
                var rpath = @"C:\Program Files\R\R-3.6.0\bin\Rscript.exe";//CAMBIAR
                var scriptpath = @"C:\Users\Família\Desktop\UnityProject_Continuous\NN_readr.R";//CAMBIAR String.Join(" ", sensorOutput.Select(x=> x.ToString()).ToArray())
                ...
                //UnityEngine.Debug.Log(double.Parse(aux[1], System.Globalization.CultureInfo.InvariantCulture)); 
                Movement.SetInputs(double.Parse(aux[1], System.Globalization.CultureInfo.InvariantCulture));
